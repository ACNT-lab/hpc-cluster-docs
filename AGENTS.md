...
## File Governance / 檔案治理

### Protocol vs State / 協定 vs 狀態

| Layer / 層級 | What it holds / 內容 | Files / 檔案 | Rules / 規則 |
|------|----------|-------|-------|
| **Protocol** | Thinking logic, rules, reference maps | `AGENTS.md`, `ACMT_HPC_Cluster_Nodes_Configuration.md`, `tools-commands.md` | Define behaviour; **no experiment records or task logs** / 定義行為邏輯，**嚴禁寫入實驗紀錄或任務流水帳** |
| **State** | Progress, live data, TODOs | `STATUS.md`, `TODO.md`, `maintenance-log.md` | Append-only for logs; overwrite for status / 紀錄用 append，狀態可覆寫 |

### Ansible-First Configuration / Ansible 優先配置

When making configuration changes, modify the Ansible-controlled source files **first** so `ansible-playbook` remains the single source of truth for deployment. Only hot-fix on live files when Ansible is not involved.

進行設定變更時，優先修改 Ansible 管理的原始檔案，確保 `ansible-playbook` 仍是部署的唯一依據。僅在非 Ansible 管轄範圍才直接修改即時設定檔。

### Documentation Principles / 文件原則

- **Concise**: One file per concern, no fragmentation / 簡潔明瞭，避免碎片化
- **Objective**: Neutral wording, state facts without editorialising / 用字客觀中性
- **Modify first**: Improve existing files before creating new ones / 優先修改現有檔案

### Git Workflow / Git 流程

- **No proactive push**: Only commit/push when explicitly asked / 不主動推送，明確要求才執行
- **Pre-commit checks**: Verify `.gitignore` and `git status` before every commit / 每次 commit 前檢查 `.gitignore` 與 `git status`
- **README**: Update `README.md` before uploading if the project structure changed / 上傳前如有專案結構變動需更新 `README.md`
...
4. **Document changes**: When making configuration changes, note what was changed and why in `maintenance-log.md`.
   **變更要文件化**：設定變更後在 `maintenance-log.md` 記錄變動內容與原因。
...
|-----------|-------|----------|---------|-------|
| r620 | acmt01-02 | 1 | No | 209GB / 250GB RAM |
| r630a | acmt04 | 1 | No | 28 cores |
| r630b | acmt05-06 | 1 | No | 24 cores |
| r630c | acmt07 | 1 | No | 20 cores |
| r630l | acmt08 | 1 | No | 28 cores, 129GB RAM |
| r630m | acmt03,12 | 1 | No | 16 cores, 161GB RAM |
| r630s | acmt09-11,13-15 | 1 | **YES** | Default partition (16-48GB/node — watch mem requests) / 預設分區（每節點 16-48GB — 注意記憶體請求） |
| apollo | acmt16-19 | 1 | No | Silver 4114 (11-16GB/node, some nodes may be offline — see STATUS.md) / Silver 4114（每節點 11-16GB，部分節點可能離線 — 見 STATUS.md） |
...
| dl360 | acmt21-26 | 1 | No | Gold 6142, 258GB RAM |
| dl360s | acmt27 | 1 | No | Single-thread, 258GB RAM / 單執行緒 |
...
Node state is volatile — always fetch live:
節點即時狀態請以指令抓取：
...
Known unresolved node issues: see [`STATUS.md`](STATUS.md) §1.1.
已知未解的節點問題：見 [`STATUS.md`](STATUS.md) §1.1。

### Accounting / 帳號管理

- Only one Slurm account: `lab` / 只有一個 Slurm account：`lab`
- Only one QoS: `normal` (Priority=0) / 只有一個 QoS：`normal`（Priority=0）
- Root is the only Administrator / root 是唯一的 Administrator
- ~15 regular users (`sacctmgr show users`; all in `lab` group)
  約 15 位一般使用者（`sacctmgr show users`；全部屬於 `lab` 群組）
- Scheduler: `sched/builtin` with multifactor priority / 排程器：`sched/builtin` 搭配 multifactor priority
...
- **NFS /home**: 11TB total, 6.4TB used (4.2TB free) — from acmt-storage
  NFS /home：總共 11TB、已使用 6.4TB（剩 4.2TB） — 來自 acmt-storage
- **NFS /opt**: Same NFS export, shared software
  NFS /opt：同一個 NFS export，存放共享軟體
- **NFS mount options**: `/home` uses `soft`, `/opt` uses `hard`; both `proto=tcp,timeo=600,retrans=2`
  NFS 掛載參數：`/home` 為 `soft`，`/opt` 為 `hard`；共用 `proto=tcp,timeo=600,retrans=2`
...
3. Edit Ansible `hosts` file if IP not in range / 若 IP 不在範圍內，編輯 Ansible `hosts` 檔
